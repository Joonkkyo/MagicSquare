...
from entity.constants import BLANK_CELL_VALUE, GRID_SIZE, MAGIC_CONSTANT, MAX_CELL_VALUE
from entity.loc import find_blank_coords
from entity.validation import sum_col, sum_row


def _all_lines_sum_to_magic(grid):
    for row in range(GRID_SIZE):
        if sum_row(grid, row) != MAGIC_CONSTANT:
            return False
    for col in range(GRID_SIZE):
        if sum_col(grid, col) != MAGIC_CONSTANT:
            return False
